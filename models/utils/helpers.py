...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=False, progress=True):
    if cfg is None:
    ...
    if in_chans == 1:
        ...
        raise ValueError("Invalid in_chans for pretrained weights. Expected in_chans = 3")

    classifier_name = cfg['classifier']
    # if hasattr(model, 'classifier') and classifier_name != 'classifier':
    #     state_dict['classifier.weight'] = state_dict[classifier_name + '.weight']
    #     state_dict['classifier.bias'] = state_dict[classifier_name + '.bias']
    ...
    #     state_dict['classifier.weight'] = classifier_weight[1:]
    #     classifier_bias = state_dict['classifier.bias']
    #     state_dict['classifier.bias'] = classifier_bias[1:]
...
def adapt_model_from_string(parent_module, model_string):
    ...
    hooks = [dict(name=name, type='forward') for name in new_module.stage_names]
    feature_hooks = FeatureHooks(hooks, new_module.named_modules())
    x = torch.rand(3, 3, 224, 224)
    new_module(x)
    new_channels = [i.size(1) for i in feature_hooks.get_output(x.device)]
