...
class CondConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        for wd in self.weight_shape:
            weight_num_param *= wd
        self.weight = torch.nn.Parameter(torch.zeros(self.num_experts, weight_num_param), requires_grad=True)

        if bias:
            self.bias_shape = (self.out_channels,)
            self.bias = torch.nn.Parameter(torch.zeros(self.num_experts, self.out_channels), requires_grad=True)
        else:
